//
// Created by mariakolyachko on 3.03.20.
//
...
struct AddressHasher {
  size_t operator() (const Address &address) const {
    const size_t BASE = 137;
    return str_hash(address.city) * BASE * BASE
      + str_hash(address.street) * BASE
...
struct PersonHasher {
  size_t operator() (const Person &person) const {
    const size_t BASE = 241;
    return str_hash(person.name) * BASE * BASE * BASE
      + int_hash(person.height) * BASE * BASE
      + dbl_hash(person.weight) * BASE
      + adr_hash(person.address);
  }

  hash<string> str_hash;
  hash<int> int_hash;
  hash<double> dbl_hash;
