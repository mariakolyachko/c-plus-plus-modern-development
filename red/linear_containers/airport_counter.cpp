...
public:
  ...
  template <typename TIterator> AirportCounter(TIterator begin, TIterator end) {
    ...
    for (auto it = begin; it != end; ++it) {
      ++airport_departure_[static_cast<size_t>(*it)];
    }
  }

  // получить количество элементов, равных данному
  [[nodiscard]] size_t Get(TAirport airport) const {
    return airport_departure_[static_cast<size_t>(airport)];
  }

  // добавить данный элемент
  void Insert(TAirport airport) {
    ++airport_departure_[static_cast<size_t>(airport)];
  }

  // удалить одно вхождение данного элемента
  void EraseOne(TAirport airport) {
    if (airport_departure_[static_cast<size_t>(airport)] > 0) {
      --airport_departure_[static_cast<size_t>(airport)];
    }
  }
  ...
  void EraseAll(TAirport airport) {
    airport_departure_[static_cast<size_t>(airport)] = 0;
  }
  ...
  [[nodiscard]] Items GetItems() const {
    ...
    for (size_t i = 0; i < static_cast<size_t>(TAirport::Last_); ++i) {
      result[i] = {static_cast<TAirport>(i), airport_departure_[i]};
    }
    return result;
  }

private:
  array<size_t, static_cast<size_t>(TAirport::Last_)> airport_departure_;
};
...
int main1() {
  ...
  { LOG_DURATION("TestMostPopulatAiport");
